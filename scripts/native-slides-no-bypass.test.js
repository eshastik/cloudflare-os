...
test('all mutations require revision or text comparison; legacy RPC cannot bypass guards',async()=>{
 ...
 const mf=new Miniflare({host:'127.0.0.1',port:0,workers:[
  ...
  {name:'driver',modules:true,compatibilityDate:'2026-02-02',durableObjects:{SLIDES:{className:'TestGadget',scriptName:'slides',useSQLite:true}},script:`
   export default {async fetch(request,env){
    ...
    const created=await g.mutateDocument(initial.revision,'addBlock',[id,{type:'text',x:10,y:20,props:{text:'base',color:'black'}}]);const blockId=created.result;
    const protectedBefore=await g.getDeck(),historyBefore=await g.getUndoState();
    const oldCalls=['undo','redo','addSlide','removeSlide','duplicateSlide','moveSlide','updateSlide','addBlock','updateBlock','removeBlock','reorderBlock','setDeck','resetAll'];
    let refused=0;for(const method of oldCalls){try{await g[method]();}catch{refused++;}}
    const bypassClosed=refused===oldCalls.length&&JSON.stringify(await g.getDeck())===JSON.stringify(protectedBefore)&&JSON.stringify(await g.getUndoState())===JSON.stringify(historyBefore);
