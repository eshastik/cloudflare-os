...
export function siteLogoDimensions(width: number, height: number): { width: number; height: number } {
  if (width <= 0 || height <= 0) throw new Error('Неверный размер картинки.')
...
function readFile(file: File, mode: 'arrayBuffer' | 'text'): Promise<ArrayBuffer | string> {
  return new Promise((resolve, reject) => {
    ...
    reader.addEventListener('error', () => reject(new Error('Не удалось прочитать картинку.')), {
      once: true,
    })
...
async function validateSvgResources(file: File): Promise<void> {
  ...
  if (root.localName !== 'svg' || document.querySelector('parsererror')) {
    throw new Error('Этот файл не похож на поддерживаемую картинку.')
  }
  ...
  for (const element of [root, ...root.querySelectorAll('*')]) {
    if (blockedElements.has(element.localName.toLowerCase())) {
      throw new Error('Логотип SVG не может содержать встроенные картинки или внешние ресурсы.')
    }
...
function validateSourceDimensions(
  ...
  if (width <= 0 || height <= 0 || (enforceRasterLimit &&
    ...
    throw new Error('Слишком большой размер картинки.')
  }
}
...
async function preflightRasterDimensions(file: File): Promise<void> {
  ...
  if (!dimensions) throw new Error('Этот файл не похож на поддерживаемую картинку.')
...
function canvasToPng(canvas: HTMLCanvasElement): Promise<Blob> {
  return new Promise((resolve, reject) => {
    canvas.toBlob((blob) => {
      ...
      else reject(new Error('Не удалось сохранить логотип как PNG.'))
    }, 'image/png')
  })
}
  ...
  try {
    const image = await new Promise<HTMLImageElement>((resolve, reject) => {
      ...
      element.addEventListener('error', () => {
        reject(new Error('Не удалось открыть файл как картинку.'))
      }, { once: true })
      element.src = url
    })
    return {
...
export async function prepareSiteLogo(file: File): Promise<Uint8Array> {
  if (file.size > MAX_SOURCE_BYTES) throw new Error('Картинка слишком большая (не больше 5 МБ).')
  if (file.type && !file.type.startsWith('image/')) throw new Error('Выберите файл с картинкой.')
  ...
  try {
    ...
    throw new Error('Не удалось открыть файл как картинку.')
  }
  ...
  try {
    ...
    if (png.size > MAX_SITE_LOGO_BYTES) {
      throw new Error('Логотип слишком сложный и не укладывается в лимит загрузки.')
    }
...
export function applySiteFavicon(logoUrl: string | undefined): () => void {
  ...
  fetch(logoUrl, { cache: 'no-cache', signal: controller.signal })
    .then((response) => {
      if (!response.ok) throw new Error(`Не удалось получить логотип (код ${response.status})`)
