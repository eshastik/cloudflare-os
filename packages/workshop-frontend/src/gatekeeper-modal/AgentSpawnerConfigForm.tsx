...
export function validateSpawnerEnv(rows: SpawnerEnvRow[]): string | null {
  ...
  for (const row of rows) {
    ...
    if (seen.has(row.name)) {
      return `Название «${row.name}» повторяется у двух подключений.`
    }
    seen.add(row.name)
  }
  return null
  ...
  return (
    <section className="grid gap-4">
      <ConnectionConfigField
        label="Название"
        description="Назовите агента для этого подключения."
      >
        <WorkshopInput
          aria-label="Название агента"
          placeholder="Например, помощник по почте"
          value={displayName}
      ...
      <ConnectionConfigField
        label="Модель"
        description="Выберите модель для запускаемых агентов."
      >
        <Select
      ...
      <ConnectionConfigField
        label="Ресурсы агента"
        description="Ресурсы, доступные агентам, и их названия для обращения из кода."
      >
        {env.length === 0 ? (
          <p className="text-[12px] leading-4 font-normal tracking-[-0.2px] text-kumo-subtle">
            Здесь пока нет ресурсов для агента. Создайте агента в разделе подключений приложения,
            чтобы дать ему доступ к приложению и его ресурсам.
          </p>
        ) : (
          <div className="grid gap-2">
            {env.map((row, index) => (
              <div key={`${row.target}:${index}`} className="flex items-center gap-2">
                <Checkbox
                  aria-label={`Дать агентам доступ к ${row.targetTitle}`}
                  checked={row.enabled}
                  onCheckedChange={(checked) => updateRow(index, { enabled: checked === true })}
                />
                <WorkshopInput
                  aria-label={`Название подключения для ${row.targetTitle}`}
                  value={row.name}
