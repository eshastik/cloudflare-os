    ...
    const loadVendors = async () => {
      ...
      try {
        ...
        if (unavailable.length > 0) {
          toasts.add({
            title: `Некоторые сервисы временно недоступны: ${unavailable.map(v => v.id).join(', ')}`,
            variant: 'warning',
          })
        }
        ...
        toasts.add({ title: 'Не удалось загрузить список сервисов', variant: 'error' })
      } finally {
  ...
  return (
    <div style={style}>
      <div className="overflow-y-auto" style={{ maxHeight }}>
        {!ready ? (
          <p className={PICKER_EMPTY}>Загрузка подключений…</p>
        ) : matchedResources.length === 0 ? (
          <p className={PICKER_EMPTY}>Ничего не найдено.</p>
        ) : (() => {
          let itemIdx = 0
              ...
              <div key={`${vendor.id}-${resource.urlPattern}`} className={i > 0 ? 'border-t border-kumo-line' : ''}>
                ...
                {vendorAccounts.map(account => {
                    ...
                    >
                      ...
                      {isReconnecting || isGranting ? (
                        ...
                        <span className="flex flex-shrink-0 items-center gap-1">
                          ...
                          <span className="text-[11.5px] leading-4 text-kumo-warning">Вход истёк — нажмите, чтобы войти заново</span>
                        </span>
                      ) : needsAccess ? (
                        <span className="flex flex-shrink-0 items-center gap-1">
                          <Warning size={12} className="text-kumo-warning" />
                          <span className="text-[11.5px] leading-4 text-kumo-warning">Выдать доступ</span>
                        </span>
                  ...
                  if (searchHasPlaceholders) {
                    return (
                      <Tooltip key={account.id} content="Сначала замените все заполнители в адресе, затем выберите аккаунт" asChild>
                        {accountRow}
                      </Tooltip>
                    )
                  }
                ...
                {(() => {
                  ...
                  >
                    ...
                    <span className="flex-1 text-[13px] leading-[18px] tracking-[-0.25px] text-kumo-subtle">
                      {connectingVendor === vendor.id ? 'Открываем…' : 'Подключить новый аккаунт'}
                    </span>
                    {isActive && <TabHint />}
                  </div>
                  )
