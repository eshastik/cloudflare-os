...
export default function Connections({ overseer, gadget, chatId, authenticatedApi, onConnectionsChange, isVisible, onHasGatekeepersChange }: ConnectionsProps) {
  ...
  const loadGatekeepers = async () => {
    try {
      ...
      toasts.add({ title: 'Не удалось загрузить подключения. Откройте вкладку ещё раз.', variant: 'error' })
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleToggleHook = async (id: number, enabled: boolean) => {
    ...
    try {
      ...
      toasts.add({ title: `Не удалось ${enabled ? 'включить' : 'выключить'} хук. Попробуйте ещё раз.`, variant: 'error' })
  ...
  const handleDeleteHookConfirm = async () => {
    ...
    try {
      ...
      toasts.add({ title: 'Не удалось удалить хук. Попробуйте ещё раз.', variant: 'error' })
    } finally {
    ...
    return [
      {
        ...
        targetTitle: `${gadgetInfo.title} (этот гаджет)`,
        name: 'GADGET',
      },
  ...
  const handleEditSave = async (name: string) => {
    ...
    if (!newName) {
      toasts.add({ title: 'Имя привязки не может быть пустым', variant: 'error' })
      return
    }
    if (newName === name) {
    ...
    try {
      ...
      toasts.add({ title: 'Не удалось изменить имя привязки. Попробуйте ещё раз.', variant: 'error' })
    } finally {
      setEditingBinding(null)
    }
  }
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      toasts.add({ title: 'Не удалось удалить подключение. Попробуйте ещё раз.', variant: 'error' })
    } finally {
  ...
  return (
    <div className="h-full overflow-auto bg-kumo-base">
      <div className="mx-auto flex min-h-full w-full max-w-5xl flex-col px-4 py-5 sm:px-6">
        <section>
          <div className="mb-3 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            <div className="min-w-0">
              ...
              <p className="mt-1 text-[13px] leading-[18px] font-normal tracking-[-0.25px] text-kumo-subtle">
                Внешние ресурсы, которые может использовать этот гаджет.
              </p>
            </div>
            <WorkshopButton
              tone="primary"
          ...
          {loading ? (
            ...
            <EmptyState
              title="Подключённых ресурсов нет"
              description="Подключите документы, репозитории, таблицы и другие сервисы, чтобы гаджет безопасно работал с внешними данными."
              actionLabel="Подключить ресурс"
                    ...
                    {isDeleting ? (
                      <div className="flex flex-wrap items-center gap-3">
                        <div className="min-w-0 flex-1">
                          <p className="truncate text-[13px] leading-[18px] font-medium tracking-[-0.25px] text-kumo-danger">
                            Удалить {gk.resourceTitle}?
                          </p>
                          <p className="truncate text-[12px] leading-4 font-normal tracking-[-0.2px] text-kumo-subtle">
                            Привязка <span className="font-mono">{gk.name}</span> будет удалена из этого гаджета.
                          </p>
                        </div>
                        <WorkshopButton
                          tone="danger"
                      ...
                      <div className="flex items-center gap-2">
                        <WorkshopInput
                          ...
                          placeholder="Имя привязки"
                          aria-label="Имя привязки"
                          autoFocus
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="min-w-0 flex-1">
                          <p className="flex items-center gap-2 truncate text-[13px] leading-[18px] font-medium tracking-[-0.25px] text-kumo-default">
                            ...
                            {isPending && (
                              <Tooltip content="Добавлено в этом чате; сохранится, когда вы примете изменения чата" asChild>
                          ...
                          <p className="mt-0.5 truncate text-[11px] leading-4 tracking-[-0.1px] text-kumo-inactive">
                            В коде доступно как: <span className="font-mono text-kumo-subtle">{gk.name}</span>
                          </p>
                        </div>
                        <div className="ml-auto flex shrink-0 items-center gap-1">
                          <Tooltip content="Изменить имя в коде" asChild>
                            <WorkshopIconButton
                              onClick={() => handleEditStart(gk.name)}
                              aria-label="Изменить имя в коде"
                            >
                              <Pencil size={14} />
                            </WorkshopIconButton>
                          </Tooltip>
                          {!isPending && (
                            <Tooltip content="Настройки шаблона" asChild>
                              <WorkshopIconButton
                          ...
                          <Tooltip content="Удалить подключение" asChild>
                            <WorkshopIconButton
                              danger
        ...
        {!loading && hooks.length > 0 && (
          <section className="mt-8">
            <div className="mb-3">
              ...
              <p className="mt-1 text-[13px] leading-[18px] font-normal tracking-[-0.25px] text-kumo-subtle">
                Обратные вызовы: подключённые ресурсы будят этот гаджет, когда происходят события.
              </p>
            </div>
                  ...
                  >
                    {isDeleting ? (
                      <div className="flex flex-wrap items-center gap-3">
                        <div className="min-w-0 flex-1">
                          <p className="truncate text-[13px] leading-[18px] font-medium tracking-[-0.25px] text-kumo-danger">
                            Удалить хук «{hook.description.title}»?
                          </p>
                          <p className="truncate text-[12px] leading-4 font-normal tracking-[-0.2px] text-kumo-subtle">
                            Хук будет удалён навсегда. Новые события перестанут приходить.
                          </p>
                        </div>
                        <WorkshopButton
                          tone="danger"
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="ml-auto flex shrink-0 items-center gap-2">
                          ...
                          />
                          <Tooltip content="Удалить хук" asChild>
                            <WorkshopIconButton
                              danger
          ...
          try {
            ...
            toasts.add({
              title: chatId === undefined
                ? 'Подключение создано'
                : 'Подключение создано — примите изменения чата, чтобы его сохранить',
              variant: 'success',
            })
            await loadGatekeepers()
            onConnectionsChange?.()
      ...
      <BlueprintAnnotationModal
        ...
        onSaved={() => {
          toasts.add({ title: 'Настройки шаблона сохранены.', variant: 'success' })
      ...
      try {
        ...
        if (!cancelled) {
          if (loaded) {
            ...
            setLoadError('Подключение не найдено.')
          }
        }
      } catch (err: any) {
        if (!cancelled) {
          reportIssue('connections.binding-load', err)
          setLoadError(err?.message || 'Не удалось загрузить привязку.')
        }
      }
    })()
    return () => {
        ...
        <Dialog className="!z-[1000] !w-[min(480px,calc(100vw-32px))] overflow-hidden bg-kumo-base p-0" size="lg">
          <div className="flex items-start justify-between gap-4 border-b border-kumo-line px-4 py-4 sm:px-5">
            <div className="min-w-0">
              ...
              <Dialog.Description className="mt-1 text-[12px] leading-4 font-normal tracking-[-0.2px] text-kumo-subtle">
                Как это подключение выглядит в шаблонах.
              </Dialog.Description>
            </div>
            <Dialog.Close
              render={(props) => (
          ...
          <div className="space-y-4 px-4 py-4 sm:px-5">
            {loadError ? (
              ...
              <div className="py-2 text-center text-[13px] text-kumo-subtle">Загрузка…</div>
            ) : (
              <>
                <BlueprintBindingCard
                  data={data}
