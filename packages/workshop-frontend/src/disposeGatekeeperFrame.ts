...
export function disposeGatekeeperFrame(frame: GatekeeperUiFrame | null) {
  for (const capability of [frame?.organizationMetrics, frame?.calendarDraftCreator, frame?.mailDraftSender, frame?.agentConsent, frame?.ui, frame?.textUploads?.issuer, frame?.textDownloads?.issuer,
