...
export function disposeGatekeeperFrame(frame: GatekeeperUiFrame | null) {
  for (const capability of [frame?.blueprintTemplates?.selector, frame?.organizationMetrics, frame?.calendarDraftCreator, frame?.mailDraftSender, frame?.agentConsent, frame?.ui, frame?.inboxUploads?.issuer, frame?.textUploads?.issuer, frame?.textDownloads?.issuer,
