...
const mocks=vi.hoisted(()=>({apply:vi.fn<(...args:unknown[])=>Promise<unknown>>(),create:vi.fn<(...args:unknown[])=>Promise<unknown>>(),navigate:vi.fn<(...args:unknown[])=>void>(),download:vi.fn<(...args:unknown[])=>Promise<Uint8Array>>()}))
...
vi.mock('./accountCapabilities',()=>({listAccounts:async()=>[{id:1,vendorId:'memory',description:{displayName:'Компания'}}],storesDocuments:()=>true,openBlueprintTemplatesFrame:async()=>({blueprintTemplates:{storageOrigin:'https://objects.example',selector:{
 scopes:async()=>({scopes:[{scope_id:'finance',revision:1,level:'department',name:'Финансы',enabled:true}]}),
 ...
 apply:(...args:unknown[])=>mocks.apply(...args),validateApplication:async()=>{}
 }}})}))
...
test('показывает уровень и создаёт рабочий гаджет из выбранной утверждённой версии',async()=>{
 await choose();expect(host.textContent).toContain('Отдел · Финансы');expect(host.textContent).toContain('Утверждённая версия 3')
