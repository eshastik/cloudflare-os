...
export default function BlueprintModal({ open, onClose, overseer, gadget, metadata }: Props) {
  ...
  const loadBlueprints = useCallback(async () => {
    ...
    try {
      ...
      toasts.add({ title: 'Не удалось загрузить шаблоны', variant: 'error' })
    } finally {
      setLoading(false)
    }
  }, [overseer])
  ...
  const loadBindings = useCallback(async () => {
    ...
    try {
      ...
      setBindingsError('Не удалось загрузить подключения.')
    } finally {
  ...
  const handleScreenshotSelected = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      toasts.add({ title: 'Не удалось обработать обложку', variant: 'error' })
    } finally {
  ...
  const createBlueprint = async () => {
    ...
    try {
      ...
      toasts.add({ title: 'Шаблон создан.', variant: 'success' })
      setFormMode('list')
      setNewTitle(metadata.title)
      setNewDescription('')
      setNewScreenshotBlob(null)
      ...
      setCreateError(err.message || 'Не удалось создать шаблон.')
    } finally {
      setCreating(false)
    }
  }
  ...
  const saveBlueprintEdits = async () => {
    ...
    try {
      ...
      toasts.add({ title: 'Шаблон обновлён.', variant: 'success' })
      setFormMode('list')
      ...
      setCreateError(err.message || 'Не удалось обновить шаблон.')
    } finally {
      setCreating(false)
    }
  }
  ...
  const deleteBlueprint = async (id: string) => {
    ...
    try {
      ...
      toasts.add({ title: err.message || 'Не удалось удалить шаблон.', variant: 'error' })
    } finally {
      setDeletingId(null)
    }
  }
          ...
          <div className="flex items-start justify-between gap-4 border-b border-kumo-line px-4 py-5 sm:px-6">
            <div className="flex min-w-0 items-start gap-3">
              ...
              <Dialog.Title className="text-[17px] leading-6 font-medium tracking-[-0.35px] text-kumo-default">
                {formMode === 'create' ? 'Создать шаблон' : formMode === 'edit' ? 'Изменить шаблон' : 'Шаблоны'}
              </Dialog.Title>
              <Dialog.Description className="mt-1 text-[13px] leading-[18px] font-normal tracking-[-0.25px] text-kumo-subtle">
                {formMode === 'create'
                  ? 'Опишите, что можно сделать с помощью этого шаблона.'
                  : formMode === 'edit'
                    ? 'Измените описание, обложку и инструкции по подключениям.'
                    : 'Сохраните гаджет как шаблон для новых задач.'}
              </Dialog.Description>
              </div>
            </div>
            <Dialog.Close
              ...
              <>
                      ...
                      <div className="flex items-center justify-between gap-3">
                        <div className="min-w-0">
                          ...
                          <p className="m-0 mt-0.5 text-[12px] leading-4 font-normal tracking-[-0.2px] text-kumo-subtle">
                            Изображение для каталога и страницы шаблона.
                            {formMode === 'edit' && !newScreenshotUrl && editingBlueprint?.screenshotUrl && !clearScreenshot ? ' Текущая обложка сохранится, пока вы не замените её.' : ''}
                          </p>
                        </div>
                        <div className="flex shrink-0 items-center gap-1.5">
                          ...
                          >
                            ...
                            {processingScreenshot ? 'Обработка…' : newScreenshotUrl || (formMode === 'edit' && editingBlueprint?.screenshotUrl && !clearScreenshot) ? 'Заменить' : 'Загрузить'}
                          </WorkshopButton>
                        </div>
                      </div>
                  ...
                  {bindingsLoading ? (
                    ...
                    <section>
                      ...
                      <p className="m-0 mb-3 text-[12px] leading-4 font-normal tracking-[-0.2px] text-kumo-subtle">
                        Укажите названия подключений и инструкции для пользователей шаблона.
                      </p>
                ...
                <div className="border-t border-kumo-line px-4 py-4 sm:px-6">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    >
                      {creating
                        ? formMode === 'create' ? 'Создание…' : 'Сохранение…'
                        : processingScreenshot ? 'Обработка…'
                          : bindingsLoading ? 'Загрузка…'
              ...
              >
                <span>
                  ...
                  <span className="mt-0.5 block text-[13px] leading-[18px] font-normal tracking-[-0.25px] text-kumo-subtle">
                    Опубликуйте гаджет как шаблон для новых задач.
                  </span>
                </span>
                    ...
                    <BlueprintRow
                      ...
                      onUpdateCode={async () => {
                        try {
                          ...
                          toasts.add({ title: 'Шаблон обновлён до текущей версии гаджета.', variant: 'success' })
                          loadBlueprints()
                        } catch (err: any) {
                          toasts.add({ title: err.message || 'Не удалось обновить шаблон.', variant: 'error' })
                        }
                      }}
                      onRetryPublish={async () => {
                        try {
                          await overseer.retryBlueprintPublish(bp.id)
                          toasts.add({ title: 'Шаблон опубликован.', variant: 'success' })
                          loadBlueprints()
                        } catch (err: any) {
                          toasts.add({ title: err.message || 'Не удалось повторить публикацию.', variant: 'error' })
                        }
                      }}
                      onCopyLink={async () => {
      ...
      >
        <div className="flex w-full flex-wrap items-center gap-3">
          <div className="min-w-0 flex-1">
            ...
            <p className="m-0 mt-0.5 text-[12px] leading-4 font-normal tracking-[-0.2px] text-kumo-subtle">
              Созданные из шаблона гаджеты сохранятся. Ссылка на шаблон перестанет работать.
            </p>
          </div>
          <button
            type="button"
    ...
    >
      <div className="flex min-w-0 flex-wrap items-center gap-3">
        ...
        <span
          ...
          title={bp.dirty ? 'Не удалось опубликовать' : undefined}
        >
          {bp.dirty && (
            <span
      ...
      <div className="mt-3 flex flex-wrap items-center justify-between gap-2">
        <div className="-ml-[7px] flex flex-wrap items-center gap-1">
          ...
          >
            {copyState === 'copied' ? 'Скопировано' : copyState === 'failed' ? 'Не удалось скопировать' : 'Копировать ссылку'}
          </GhostButton>
        </div>
