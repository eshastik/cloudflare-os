...
const API_TOKEN_PLACEHOLDERS: Record<AiModelProvider, string> = {
  ...
  openai: 'sk-...',
  google: 'AIza...',
  cloudflare: 'Ключ API Cloudflare',
...
function buildOptions(gatewayMode: boolean, enabledProviders: Set<string> | null) {
  ...
  for (const provider of providerOrder) {
    ...
    options.push({
      ...
      label: `Другая модель ${PROVIDER_LABELS[provider] || provider}…`,
      provider,
    })
  }
...
export default function AddModelModal({ visible, onCancel, onSuccess, authenticatedApi, aiConfig }: AddModelModalProps) {
  ...
  const validate = (): boolean => {
    ...
    if (!selection) {
      newErrors.selection = gatewayMode ? 'Выберите поставщика' : 'Выберите модель'
    }

    if (selection?.type === 'custom') {
      if (!modelId.trim()) newErrors.modelId = 'Укажите идентификатор модели'
      if (!displayName.trim()) newErrors.displayName = 'Укажите название'
    }
    ...
    if (showCredentials && selection && !isOllama && !apiToken.trim()) {
      newErrors.apiToken = 'Укажите ключ API'
    }

    if (showCredentials && isCloudflare && !accountId.trim()) {
      newErrors.accountId = 'Укажите идентификатор аккаунта Cloudflare'
    }

    if (showCredentials && isOllama && !apiUrl.trim()) {
      newErrors.apiUrl = 'Укажите адрес сервера Ollama'
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toasts.add({ title: 'Модель добавлена', variant: 'success' })
      onSuccess()
    } catch (error: any) {
      console.error('Failed to add model:', error)
      toasts.add({ title: 'Не удалось добавить модель', variant: 'error' })
    } finally {
      setLoading(false)
    }
  }
    ...
    <Dialog.Root open={visible} onOpenChange={(open) => { if (!open) onCancel() }}>
      <Dialog className="p-6" size="lg">
        ...
        <div className="space-y-4">
          ...
          <Select
            label={gatewayMode ? 'Поставщик' : 'Модель'}
            className="w-full text-sm"
            placeholder={gatewayMode ? 'Выберите поставщика…' : 'Выберите модель…'}
            value={selectValue}
          ...
          {showCustomFields && (
            <>
              <Input
                ...
                placeholder={`например, ${example!.modelId}`}
                description={`Идентификатор модели так, как его пишет поставщик (например, «${example!.modelId}»)`}
                value={modelId}
              ...
              <Input
                label="Название"
                placeholder={`например, ${example!.name}`}
                description="Название, которое увидят пользователи"
                value={displayName}
          ...
          {showCredentials && isCloudflare && (
            <Input
              label="Идентификатор аккаунта Cloudflare"
              placeholder="например, 0123456789abcdef0123456789abcdef"
              description="Аккаунт Cloudflare, на который пойдёт оплата Workers AI"
              value={accountId}
          ...
          {showCredentials && selection && (
            <SensitiveInput
              ...
              description={
                isOllama
                  ? 'Для локального Ollama необязателен'
                  : isCloudflare
                  ? 'Ключ API с правами Workers AI на чтение и изменение (в панели Cloudflare: Workers AI > Use REST API > Create a Workers AI API Token)'
                  : `Ваш ключ API ${PROVIDER_LABELS[selection.provider]}, на него пойдёт оплата`
              }
              value={apiToken}
          ...
          {showCredentials && selection && !isOllama && !isCloudflare && (
            ...
            >
              <Collapsible.DefaultTrigger>Дополнительно</Collapsible.DefaultTrigger>
              <Collapsible.DefaultPanel>
                <Input
                  label="Адрес API"
                  placeholder="https://..."
                  description="Другой адрес API вместо стандартного (например, для прокси вроде Cloudflare AI Gateway)"
                  value={apiUrl}
