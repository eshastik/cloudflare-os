...
describe('ShareModal', () => {
  ...
  it('reveals the workspace link to send after a direct invite', async () => {
    ...
    expect(rendered.textContent).toContain('Добавлен участник: Ada')
  ...
  it('copies the plain workspace link, never a share-link secret', async () => {
    ...
    await click(button(rendered, 'Скопировать ссылку'))

    expect(copyToClipboard).toHaveBeenCalledWith(WORKSPACE_URL)
    expect(rendered.textContent).toContain('Ссылка скопирована')
  })
  ...
  it('keeps invite and share-link requirements tied to their own role pickers', async () => {
    ...
    await click(button(rendered, 'Создать ссылку доступа'))
    ...
    await click(button(rendered, 'Создать ссылку'))
    expect(verificationSection(rendered, 'link-verification-heading').textContent)
      .toContain('Pipeline dashboard')
  })
  ...
  it('does not rename a share link when its name did not change', async () => {
    ...
    await click(button(rendered, 'Переименовать Team link'))
    expect(rendered.querySelector<HTMLInputElement>('input[aria-label="Название ссылки"]')?.value)
      .toBe('Team link')
    await click(button(rendered, 'Сохранить'))
