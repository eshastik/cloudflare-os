...
export const Route = createFileRoute('/gatekeepers_/$appId')({
  validateSearch: (search: Record<string, unknown>): {section?:string; project?:string; view?:string; document?:string; account?:number; tool?:'connections'|'summary'} => ({
    ...
    // Документ из ссылки «поделились с вами»: приложение откроет его, если доступ есть.
    document: typeof search.document === "string" && /^[A-Za-z0-9_-]{1,255}$/.test(search.document) ? search.document : undefined,
