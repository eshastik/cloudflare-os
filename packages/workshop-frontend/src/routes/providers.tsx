...
function ModelRow({
  ...
  return (
    <div
      ...
      title={isQuick ? 'Быстрая модель. Нажмите, чтобы снять' : 'Нажмите, чтобы сделать быстрой моделью'}
        ...
        <DropdownMenu>
          ...
          <DropdownMenu.Content className={MENU_CONTENT}>
            <DropdownMenu.Item onClick={onSetQuick} className={MENU_ITEM}>
              ...
              {isQuick ? 'Снять отметку «быстрая»' : 'Сделать быстрой моделью'}
            </DropdownMenu.Item>
            {!isBuiltIn && (
...
function ProvidersPage() {
  ...
  const handleDelete = async (model: AiChatAuthorInfo) => {
    if (!confirm(`Удалить «${model.name}»? Отменить это нельзя.`)) return
    ...
    try {
      ...
      toasts.add({ title: 'Не удалось удалить модель', variant: 'error' })
    } finally {
      setDeletingId(null)
    }
  }
  ...
  const handleSetQuick = async (modelId: string) => {
    ...
    try {
      ...
      toasts.add({ title: 'Не удалось изменить быструю модель', variant: 'error' })
    }
  }
    ...
    <div className="mx-auto flex h-full w-full max-w-4xl flex-col px-6 sm:px-10">
      <header className="flex items-end justify-between gap-4 px-3 pb-3 pt-10">
        <div className="min-w-0">
          <h1 className="text-2xl font-semibold tracking-tight text-kumo-default">Модели</h1>
          <p className="mt-1 text-[13px] leading-[18px] tracking-[-0.25px] text-kumo-subtle">
            Модели, которые доступны в беседах.
          </p>
        </div>
      ...
      <div className="chat-panel flex min-h-0 flex-1 flex-col gap-0.5 overflow-y-auto pt-1 pb-16">
          ...
          <div className="flex flex-col gap-2.5 px-3 pb-2">
            {gatewayMode && (
              <Notice>
                ...
                <span>
                  <strong className="font-medium text-kumo-default">Режим AI Gateway:</strong> встроенными
                  моделями управляет установка. Свои модели со своими ключами API добавить всё равно
                  можно.
                </span>
              </Notice>
            )}
        ...
        {loading ? (
          ...
          <div className="py-12 text-center text-sm">
            <p className="text-kumo-danger">Не удалось загрузить модели.</p>
          ...
          <div className="flex flex-col items-center gap-3 px-3 py-16 text-center">
            ...
            <div>
              <p className="text-sm font-medium text-kumo-default">Моделей пока нет</p>
              <p className="mt-1 text-[13px] leading-[18px] text-kumo-subtle">
                Добавьте модель, чтобы начать работу в беседах.
              </p>
            </div>
            <button type="button" onClick={() => setSheetOpen(true)} className={PRIMARY_BTN}>
              <Plus size={14} weight="bold" />
          ...
          <div className="py-12 text-center text-sm text-kumo-inactive">Модели не найдены</div>
        ) : (
          filtered.map((model) => (
            <div
              key={model.id}
