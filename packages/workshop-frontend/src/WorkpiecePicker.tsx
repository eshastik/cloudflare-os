  ...
  return (
    ...
    >
      <button
        ...
        title={expanded ? 'Свернуть файлы' : 'Развернуть файлы'}
        aria-label={expanded ? 'Свернуть файлы' : 'Развернуть файлы'}
      ...
      <div className="flex flex-1 flex-col gap-0.5 overflow-y-auto overflow-x-hidden px-1.5 pb-2">
        {gadgets.map(gadget => {
          ...
          return (
            ...
            >
              <Tooltip content={`${gadget.title}${!expanded && isPending ? ' (черновик)' : ''}${hasHook ? ' · хуки включены' : ''}`} asChild>
                <button
                  type="button"
        ...
        <Tooltip content="Открыть журнал" asChild>
          <button
            type="button"
          ...
          >
            ...
            {expanded && <span className="min-w-0 flex-1 truncate">Журнал</span>}
            {expanded ? (
