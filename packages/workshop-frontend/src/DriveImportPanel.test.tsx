...
it.each(['google','yandex','mnemos'])('%s restores an unconfirmed capture after remount without mixing owners',async(vendor)=>{
 ...
 state.api.subscribeConnectedAccounts.mockImplementation(async s=>{for(const [id,accountVendor] of [[7,vendor],[8,'mnemos']] as const)s.add(id,{displayName:accountVendor},{},[],true,accountVendor);return {[Symbol.dispose]:disposed}})
 ...
 try{
  await act(async()=>root.render(<DriveImportPanel/>));await fill('Drive source account','7');await fill('Drive Mnemos account',vendor==='mnemos'?'7':'8');await fill('Drive project','project');await fill('Drive file','file')
  if(vendor==='mnemos')await fill('Drive WebDAV account',webdav)
  await click('Сохранить копию');expect(container.textContent).toContain('Сохранение не подтверждено')
  const original=state.api.captureDriveImport.mock.calls[0];expect(original.slice(0,4)).toEqual([7,vendor==='mnemos'?7:8,vendor==='mnemos'?webdav+':file':'file','project']);expect(readDriveAttempt(sessionStorage,'owner')?.request).toBe(original[4])
