...
export function ProjectChips({ projects, onChange, loadChoices, disabled = false, trailing }: ProjectChipsProps) {
  ...
  return (
    <div ref={rootRef} className="relative flex flex-wrap items-center gap-1.5 px-5 pt-2 text-[13px] leading-4" aria-label="Проекты беседы">
      {projects.length === 0 && (
        <span className="text-kumo-subtle">Проект определится по задаче</span>
      )}
      {projects.map((project) => (
        <span
          key={`${project.accountId}:${project.projectId}`}
          className="inline-flex h-[30px] max-w-[220px] items-center gap-1 rounded-full bg-kumo-tint pl-3 pr-1.5 text-kumo-default"
        ...
        >
          ...
          <button
            ...
            className="flex h-5 w-5 cursor-pointer items-center justify-center rounded-full text-kumo-inactive hover:bg-kumo-fill hover:text-kumo-default disabled:cursor-not-allowed disabled:opacity-40"
      ...
      <button
        ...
        onClick={() => setOpen((v) => !v)}
        className="inline-flex h-[30px] w-[30px] cursor-pointer items-center justify-center rounded-full border border-dashed border-kumo-fill-hover text-kumo-subtle hover:bg-kumo-tint hover:text-kumo-default disabled:cursor-not-allowed disabled:opacity-40"
      ...
      {open && (
        <div role="listbox" aria-label="Выбор проекта" className="absolute bottom-full left-5 z-20 mb-1 max-h-64 w-72 overflow-auto rounded-2xl border border-kumo-fill bg-kumo-overlay p-1 shadow-lg">
          {failed ? (
            ...
            <button
              ...
              className="flex w-full cursor-pointer items-center justify-between gap-2 rounded-lg px-2.5 py-1.5 text-left text-[13px] leading-5 text-kumo-default hover:bg-kumo-tint"
            >
