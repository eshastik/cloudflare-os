      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2">
          ...
          <h3 className="truncate text-sm font-medium text-kumo-default">
            {item.title || 'Приложение без названия'}
          </h3>
        </div>
        {item.description && (
        ...
        <DropdownMenu>
          ...
          <DropdownMenu.Content className={MENU_CONTENT}>
            <DropdownMenu.Item onClick={() => onTogglePin(item)} className={MENU_ITEM}>
              ...
              {item.pinned ? 'Убрать из избранного' : 'В избранное'}
            </DropdownMenu.Item>
            {item.inLibrary && (
...
export default function BlueprintList() {
    ...
    Promise.all([authenticatedApi.listOwnBlueprints(), authenticatedApi.listLibraryBlueprints()])
      .then(([own, library]) => {
        ...
        const ensure = (id: string): BlueprintItem => {
          let it = map.get(id)
          if (!it) {
            it = { id, title: 'Приложение без названия', description: '', recency: 0, pinned: false, inLibrary: false, isOwn: false }
            map.set(id, it)
          }
          return it
        }
  ...
  const handleBlueprintSelected = useCallback(async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      toasts.add({ title: 'Приложение загружено', variant: 'success' })
      load()
    } catch (err) {
      console.error('Не удалось загрузить приложение:', err)
      toasts.add({ title: 'Не удалось загрузить приложение', variant: 'error' })
    } finally {
      setUploading(false)
    }
  ...
  const handleRemoveFromLibrary = async (item: BlueprintItem) => {
    try {
      ...
      toasts.add({ title: 'Приложение убрано из сохранённых', variant: 'success' })
    } catch (err) {
      console.error('Не удалось убрать приложение from library:', err)
      toasts.add({ title: 'Не удалось убрать приложение', variant: 'error' })
    }
  }
        ...
        <div className="mb-4 flex items-center gap-2 px-3">
          ...
          <div className="grid shrink-0 grid-cols-2 gap-2">
            ...
            <button
              ...
              disabled={uploading}
              title="Загрузить архив приложения (.gadget)"
      ...
      <div className="chat-panel flex min-h-0 flex-1 flex-col gap-0.5 overflow-y-auto pt-1">
        {loading ? (
          ...
          <div className="py-12 text-center text-sm">
            <p className="text-kumo-danger">Не удалось загрузить приложения.</p>
            <button type="button" onClick={load} className="mt-1 text-kumo-brand underline">Повторить</button>
          </div>
        ) : filtered.length === 0 ? (
          search ? (
            <div className="py-12 text-center text-sm text-kumo-inactive">Приложения не найдены</div>
          ) : (
            <div className="flex flex-col items-center gap-3 px-3 py-16 text-center">
              ...
              <div>
                <p className="text-sm font-medium text-kumo-default">Пока нет сохранённых приложений</p>
                <p className="mt-1 text-[13px] leading-[18px] text-kumo-subtle">
                  Сохраните собственную работу как шаблон или выберите приложение в каталоге.
                </p>
              </div>
              <div className="grid grid-cols-2 gap-2">
                ...
                >
                  <UploadSimple size={14} weight="bold" />
                  {uploading ? 'Загрузка…' : 'Загрузить .gadget'}
                </button>
              </div>
            </div>
          )
