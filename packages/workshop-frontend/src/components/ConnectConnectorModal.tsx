...
export default function ConnectConnectorModal({
  ...
  const headerTitle = isManage
    ...
    : `Подключить ${vendorDescription.displayName}`
      ...
      <span className="truncate">
        {credentialsValid
          ? accountDescription?.uniqueName
            ...
            : accountDisplayName
          : 'Срок доступа истёк; переподключите на странице «Подключения»'}
      </span>
    </div>
  ) : (
        ...
        <div className="new-gatekeeper-scroll-balanced min-h-0 flex-1 overflow-y-auto px-5 py-4">
                    ...
                    >
                      ...
                      {grantable && (
                        <Switch
                          ...
                          aria-label={
                            isManage
                              ? `Выдать доступ: ${resource.title}`
                              : `Включить: ${resource.title}`
                          }
                          checked={checked}
                          disabled={disabled}
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="text-[12px] leading-[17px] font-normal tracking-[-0.2px] text-kumo-default">
                  <span className="font-medium">
                    Гейткипер стоит между {vendorDescription.displayName} и вашими гаджетами.
                  </span>{' '}
                  <span className="text-kumo-subtle">
                    Каждый гаджет видит только подключённые ресурсы. В общем пространстве
                    гейткипер проверяет права других пользователей до того, как дать им доступ
                    к этим ресурсам.
                  </span>
                </div>
              </div>
          ...
          {isManage && (
            <div className="mt-5 rounded-lg border border-kumo-line bg-kumo-elevated px-4 py-3 text-[12px] leading-[17px] font-normal tracking-[-0.2px] text-kumo-subtle">
              Этот аккаунт могут использовать гаджеты, к которым вы его подключите. Другим
              пользователям общего пространства нужны права на подключённые ресурсы.
            </div>
          )}
        </div>
        ...
        <div className="shrink-0 flex items-center justify-between gap-3 border-t border-kumo-line bg-kumo-base px-5 py-3">
          {isManage && confirmingDisconnect ? (
            <p className="m-0 min-w-0 flex-1 text-[12px] leading-4 font-normal tracking-[-0.2px] text-kumo-default">
              Отключить {vendorDescription.displayName}? Гаджеты, которые им пользуются, потеряют доступ.
            </p>
          ) : isManage && hasPending ? (
            <p className="m-0 min-w-0 flex-1 text-[12px] leading-4 font-normal tracking-[-0.2px] text-kumo-subtle">
              Ресурсов к добавлению: {pendingPatterns.length}
            </p>
                ...
                {confirmingDisconnect ? (
                  ...
                  <>
                    ...
                    >
                      {ensuringBusy
                        ? 'Открываем…'
                        : `Перейти в ${vendorDescription.displayName}`}
                    </WorkshopButton>
                  </>
                ) : (
                  <>
              ...
              <>
                ...
                >
                  {autoProvisions
                    ? connecting
                      ? 'Добавляем…'
                      : `Добавить ${vendorDescription.displayName}`
                    : connecting
                    ? 'Открываем…'
                    : `Перейти в ${vendorDescription.displayName}`}
                </WorkshopButton>
