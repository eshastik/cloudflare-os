    ...
    >
      ...
      >
        <div className="flex items-start justify-between gap-4 border-b border-kumo-line px-5 py-4">
          <div className="min-w-0">
            <Dialog.Title className="text-[15px] leading-5 font-medium tracking-[-0.3px] text-kumo-default">
              Всегда разрешать «{actionLabel}»?
            </Dialog.Title>
            <Dialog.Description className="mt-1 text-[12px] leading-4 font-normal tracking-[-0.2px] text-kumo-subtle">
              Дальше действия «<span className="font-medium text-kumo-default">{actionLabel}</span>» в{' '}
              <span className="font-medium text-kumo-default">{resourceTitle}</span> будут выполняться
              без вопроса. Текущее действие тоже выполнится сейчас.
            </Dialog.Description>
          </div>
          <Dialog.Close
            render={(props) => (
        ...
        <div className="flex items-center justify-end gap-2 border-t border-kumo-line bg-kumo-base px-5 py-3">
          ...
          >
            {isProcessing ? 'Включаю…' : 'Всегда разрешать'}
          </WorkshopButton>
        </div>
      </Dialog>
    </Dialog.Root>
