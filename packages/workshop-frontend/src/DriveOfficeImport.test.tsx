...
const state=vi.hoisted(()=>({api:{getGatekeeperApp:vi.fn(),subscribeConnectedAccounts:vi.fn<(s:ConnectedAccountsSubscriber)=>Promise<Disposable>>(async s=>{s.add(8,{displayName:'Память',avatar:{url:''},providesUi:{title:'Память'}},{displayName:'Память',url:'https://memory.example'},[{urlPattern:'https://memory.example/drive',description:'',title:'',receives:'drive'}],true,'memory');s.ready();return {[Symbol.dispose](){}}})},render:vi.fn(),update:vi.fn()}))
...
it('opens the selected Mnemos account and passes the captured version to existing import review',async()=>{
 ...
 try{
  ...
  expect(state.api.getGatekeeperApp).toHaveBeenCalledWith('memory',8)
