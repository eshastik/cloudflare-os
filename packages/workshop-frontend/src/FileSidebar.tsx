...
export default function FileSidebar({
  ...
  const handleCreateFile = () => {
    if (!newFileName.trim()) {
      toasts.add({ title: 'Укажите имя файла', variant: 'error' })
      return
    }

    if (files.includes(newFileName.trim())) {
      toasts.add({ title: 'Файл с таким именем уже есть', variant: 'error' })
      return
    }
  ...
  const startDelete = (filename: string) => {
    if (files.length <= 1) {
      toasts.add({ title: 'Нельзя удалить единственный файл', variant: 'error' })
      return
    }
  ...
  return (
    <div className="flex h-full w-[244px] flex-col border-r border-kumo-line bg-kumo-base">
      ...
      <DeleteConfirmationDialog
        ...
        title="Удалить файл?"
        description={<>Файл <span className="font-mono text-kumo-default">{deletingFile}</span> будет удалён из приложения. Отменить это нельзя.</>}
    ...
    >
      ...
      {isRenaming ? (
        <input
          ...
          autoCapitalize="off"
          autoCorrect="off"
          aria-label={`Переименовать ${filename}`}
        ...
        >
          ...
          {isStreamingActive && (
            <span
              ...
              aria-label={`${filename}: идёт правка`}
              title="Агент правит этот файл"
            />
          )}
        </button>
      )}
