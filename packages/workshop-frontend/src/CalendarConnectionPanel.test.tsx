...
it('allows correcting preparation failure and retains a pending registration on retry',async()=>{
 ...
 try{
  ...
  await fill('Аккаунт календаря','7');await fill('Аккаунт Mnemos','8');await fill('Проект Mnemos','project');await fill('ID календаря','wrong')
...
it.each(['microsoft','mnemos'])('selects an owned %s calendar before transferring authority',async(sourceVendor)=>{
 vi.clearAllMocks();sessionStorage.clear()
 api.subscribeConnectedAccounts.mockImplementation(async s=>{for(const [id,vendor] of [[9,sourceVendor],[8,'mnemos']] as const)s.add(id,{displayName:vendor},{},[],true,vendor);return {[Symbol.dispose]:()=>{}}})
 ...
 try{
  await act(async()=>root.render(<CalendarConnectionPanel/>))
  await fill('Аккаунт календаря','9');await fill('Аккаунт Mnemos',sourceVendor==='mnemos'?'9':'8');await fill('Проект Mnemos','project')
  ...
  expect(api.prepareCalendarConnection.mock.calls[0].slice(0,4)).toEqual([9,sourceVendor==='mnemos'?9:8,'team-calendar','project'])
