...
test('TLS IMAP reading, changing membership, and revocation', {timeout:15000}, () => {
  const directory=mkdtempSync(join(tmpdir(),'mnemos-imap-'));
  try {
    ...
    const child=spawnSync(process.execPath,[new URL('./protocol.mjs',import.meta.url).pathname,cert,key],{
