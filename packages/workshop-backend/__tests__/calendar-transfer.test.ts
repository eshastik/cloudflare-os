...
it('transfers only owned accounts and retains current policy checks around saved-source reads',async()=>{
 ...
 const mnemos={getSupportedResources:async()=>[{urlPattern:'mnemos://caldav/calendars/*',title:'CalDAV',description:'',receives:'calendar' as const}],acceptCalendarReadSource:async(project:string,request:string,key:string,received:CalendarReadSource)=>{acceptCalls++;expect([project,request]).toEqual(['project','request']);expect(key).toBe('[7,"account-calendar-generation"]');saved=received;await received.validate();return {selection_id:'selection',title:'Team'};}} as Fetcher<GatekeeperUser>;
...
it('registers through an owned, valid, enabled Mnemos account only',async()=>{
 ...
 const calls:unknown[][]=[];
 const account={getSupportedResources:async()=>[{urlPattern:'mnemos://caldav/calendars/*',title:'CalDAV',description:'',receives:'calendar' as const}],registerCalendarSelection:async(...args:unknown[])=>{calls.push(args);return {connection_id:'connection'};}};
 const user=Object.create(UserDurableObject.prototype) as UserDurableObject;
 Object.assign(user,{
  env:{BLUEPRINTS:{get:async()=>serializeAdminConfig(config)}},
...
it('lists Outlook calendars only for the owner and rechecks account and policy after provider IO',async()=>{
 ...
 const account={getSupportedResources:async()=>[],listCalendars:async()=>{calls++;hook();return {calendars:[{id:'child',name:'Team'}],truncated:false}}};
 const user=Object.create(UserDurableObject.prototype) as UserDurableObject;
...
it.each(['google','microsoft','mnemos'])('creates only in the saved owned %s calendar and rechecks policy on the writer',async(vendor)=>{
 ...
 const provider={getSupportedResources:async()=>[vendor==='mnemos'?{urlPattern:'mnemos://caldav/calendars/*',title:'CalDAV',description:'',receives:'calendar' as const}:{urlPattern:resource,title:'Calendar',description:''}],getCalendarWriteSource:async(calendar:string)=>{expect(calendar).toBe('team');return {source,sourceKey:key,resource:{urlPattern:resource}}}};
 const receiver={getSupportedResources:async()=>[{urlPattern:'mnemos://caldav/calendars/*',title:'CalDAV',description:'',receives:'calendar' as const}],prepareCalendarDraftCreate:async()=>({sourceKey:'[7,"saved-generation"]',calendar_id:'team'}),createCalendarDraft:async(id:string,hash:string,saved:string,cap:typeof source)=>{expect([id,hash,saved]).toEqual(['draft','a'.repeat(64),'[7,"saved-generation"]']);return {state:'created',...await cap.create()}}};
...
it('CalDAV can transfer into the same owned Mnemos account without relaxing ownership checks',async()=>{
 ...
 let accepted=0;
 const account={getSupportedResources:async()=>[{urlPattern:'mnemos://caldav/calendars/*',title:'CalDAV',description:'',receives:'calendar' as const}],getCalendarReadSource:async()=>({source,sourceKey:'private-generation',resource:{urlPattern:'mnemos://caldav/calendars/*'}}),acceptCalendarReadSource:async(project:string,request:string,key:string,cap:typeof source)=>{expect([project,request,key]).toEqual(['project','request','[8,"private-generation"]']);await cap.validate();accepted++;return {selection_id:'selected',title:'Team'}}};
...
const ownCalendars={urlPattern:'memory://calendars/*',title:'Calendars',description:''};
const calendarReceiver=(declared:boolean)=>({
 getSupportedResources:async()=>[{...ownCalendars,...(declared?{receives:'calendar' as const}:{})}],
 acceptCalendarReadSource:async()=>({selection_id:'selected',title:'Team'}),
 registerCalendarSelection:async()=>({connection_id:'connection'}),
 listCalendars:async()=>({calendars:[{id:'own',name:'Own'}],truncated:false}),
});
it('hands calendar sources to any vendor declaring a receiving resource and refuses an undeclared one apart from an unavailable account',async()=>{
 let config=DEFAULT_ADMIN_CONFIG;
 const source={validate:async()=>{},metadata:async()=>({provider:'google',calendar_id:'calendar',title:'Team',time_zone:'UTC'}),readWindow:async()=>({calendar_id:'calendar',time_zone:'UTC',events_json:'[]',truncated:false})};
 const google={getCalendarReadSource:async()=>({source,sourceKey:'generation',resource:{urlPattern:'https://calendar.google.com/calendar/*',title:'Calendar',description:''}})};
 const user=Object.create(UserDurableObject.prototype) as UserDurableObject;
 Object.assign(user,{env:{BLUEPRINTS:{get:async()=>serializeAdminConfig(config)}},storage:{connectedAccounts:{get:(id:number)=>id===7?{id,account:google,vendorId:'google'}:id===8?{id,account:calendarReceiver(true),vendorId:'memory'}:id===9?{id,account:calendarReceiver(false),vendorId:'other'}:undefined}},ctx:{id:{toString:()=> 'user-id'},exports:{CalendarSourceLease:({props}:any)=>new CalendarSourceGuard(props.source,()=>user.checkCalendarSourceAccounts(props.accounts))}}});
 await expect(user.prepareCalendarConnection(7,8,'calendar','project','request')).resolves.toMatchObject({selection_id:'selected'});
 await expect(user.registerCalendarSelection(8,'project','request','selection')).resolves.toEqual({connection_id:'connection'});
 await expect(user.listCalendars(8)).resolves.toMatchObject({calendars:[{id:'own'}]});
 config={...DEFAULT_ADMIN_CONFIG,disabledResources:{memory:[ownCalendars.urlPattern]}};
 await expect(user.listCalendars(8)).rejects.toThrow('disabled');
 config=DEFAULT_ADMIN_CONFIG;
 const refused=await user.prepareCalendarConnection(7,9,'calendar','project','request').catch((error:Error)=>error.message);
 expect(refused).toBe('This account does not receive calendar sources.');
 await expect(user.registerCalendarSelection(9,'project','request','selection')).rejects.toThrow('does not receive calendar sources');
 await expect(user.listCalendars(9)).rejects.toThrow('Calendars unavailable.');
 await expect(user.prepareCalendarConnection(7,99,'calendar','project','request')).rejects.toThrow('unavailable');
});
