...
export default function MemoryPage({ legacy }: { legacy: HTMLElement }) {
  ...
  useEffect(() => {
    let cancelled = false;
    legacy.hidden = true;
    void Promise.all([host.getSelectedSection().catch(() => ""), host.getSelectedProject().catch(() => "")]).then(([selected, project]) => {
      if (cancelled) return;
      setSelectedProject(project); setDocumentsProject(project);
  ...
  return <div className="mx-auto flex w-full max-w-[1120px] flex-col px-4 py-6 sm:px-8 sm:py-8">
    ...
    {denied ? <p role="status">{data.projectsLoading ? "Проверка доступа…" : "Этот раздел недоступен с вашими текущими полномочиями."}</p> : <LegacySwitch state={tools}>
      ...
      {section === "intake" && <IntakeTab data={data} />}
      ...
      {section && <SectionTools section={section} onOpen={tools.open} />}
    </LegacySwitch>}
  </div>;
}
