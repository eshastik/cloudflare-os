...
export default function IntakeTab({data,compact=false}:{data:MemoryData;compact?:boolean}) {
 ...
 if(!data.identity)return <Notice tone={data.projectsError?"danger":undefined}>{data.projectsError||"Проверяем доступ к приёмной…"}</Notice>;
 ...
 return <section aria-label="Приём данных">
  <Block title={compact?"Приём материалов":"Добавить материалы организации"} actions={<Button disabled={loading||busy} variant="secondary" onClick={()=>void reload()}>Обновить</Button>}>
   {!compact&&<p>Загрузите файлы или папку. Mnemos разберёт материалы и предложит размещение. Создавать проекты заранее не нужно.</p>}
  ...
  {!!files.length&&<Block title="Последняя загрузка" count={files.length}><RowList>{[...files.filter(file=>file.error),...files.filter(file=>!file.error)].slice(0,100).map((file,i)=><Row key={i}><RowText title={file.path} note={file.error || file.refused?.detail || (file.stopped?"Не загружался: загрузка остановлена":file.receipt?.enqueued?"Принят, ожидает разбора":"Приём подтверждён")} />{file.error&&<span className="text-kumo-danger">Не подтверждён</span>}{file.refused&&<span className="text-kumo-subtle">Не принят</span>}</Row>)}</RowList>{files.length>100&&<p className="text-sm text-kumo-subtle">Показаны 100 из {files.length}: сначала неподтверждённые.</p>}</Block>}
  ...
  <Block title={decided?"Решённые вопросы":"Требуют вашего решения"} count={alerts?.alerts.length} actions={<Button variant="secondary" disabled={loading||busy} onClick={()=>setDecided(!decided)}>{decided?"Открытые вопросы":"История решений"}</Button>}>
   ...
   {decided&&alerts&&<IntakeHistory alerts={alerts.alerts} projects={projects}/>}
   {!decided&&alerts?.alerts.filter(alert=>!alert.blob_sha256_hex).map(alert=><div key={alert.id} className="py-3"><h3 className="mt-0 text-sm">{REASONS[alert.reason]??"Материал требует проверки"}</h3><Notice>Вопрос относится к внешней базе. Настройте её в разделе «Источники».</Notice></div>)}
