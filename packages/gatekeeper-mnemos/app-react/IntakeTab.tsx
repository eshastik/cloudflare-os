...
export default function IntakeTab({data,compact=false}:{data:MemoryData;compact?:boolean}) {
 ...
 return <section aria-label="Приём данных">
  ...
  {!!files.length&&<Block title="Последняя загрузка" count={files.length}><RowList>{[...files.filter(file=>file.error),...files.filter(file=>!file.error)].slice(0,100).map((file,i)=><Row key={i}><RowText title={file.path} note={file.error || (file.receipt?.enqueued?"Принят, ожидает разбора":"Приём подтверждён")} />{file.error&&<span className="text-kumo-danger">Не подтверждён</span>}</Row>)}</RowList>{files.length>100&&<p className="text-sm text-kumo-subtle">Показаны 100 из {files.length}: сначала неподтверждённые.</p>}</Block>}
