...
async function render(ui,props=data){const el=document.createElement('div');document.body.append(el);const root=createRoot(el);await act(async()=>{root.render(h(HostProvider,{value:{ui,host:{ui}}},h(PeopleTab,{data:props})));});return {el,async close(){await act(async()=>root.unmount());el.remove();}};}
...
test('Отзыв требует предварительного итога и сохраняет точную предметную область',async()=>{
 ...
 const view=await render(ui);await click(view.el,'Открыть карточку: Иван');assert.match(view.el.textContent,/Юридическая/);assert.match(view.el.textContent,/Финансовая/);
...
test('область назначения берётся из материалов проекта, а не из роли сотрудника',async()=>{
 ...
 const view=await render(ui);await click(view.el,'Открыть карточку: Иван');
 assert.equal(view.el.querySelector('[aria-label="Новое назначение"]'),null,'форма не загромождает карточку');
...
test('карточка показывает назначения без технических параметров и постоянно открытой формы',async()=>{
 ...
 const view=await render(ui);await click(view.el,'Открыть карточку: Иван');
 assert.equal(view.el.querySelector('[aria-label="Новое назначение"]'),null);
 assert.doesNotMatch(view.el.textContent,/Показаны прямые назначения|узел:|весь проект/);
 await click(view.el,'Дать доступ');assert.ok(view.el.querySelector('[aria-label="Новое назначение"]'));
 await click(view.el,'Отмена');assert.equal(view.el.querySelector('[aria-label="Новое назначение"]'),null);
 await view.close();
});
...
test('ошибка сохранения остаётся после обновления назначений; все области передаются явно',async()=>{
 ...
 const view=await render(ui);await click(view.el,'Открыть карточку: Иван');await click(view.el,'Дать доступ');
