...
type Purpose='login'|'audit';
/** One DO alarm, separate durable deadlines. Audit retries never cancel login. */
export class AccountAlarms {
 ...
 hasDeadlines(){return ['login','audit'].some(p=>this.kv.get('accountAlarm:'+p)!==undefined);}
 async #arm(){
  const times=['login','audit'].map(p=>this.kv.get<number>('accountAlarm:'+p)).filter((n):n is number=>n!==undefined);
